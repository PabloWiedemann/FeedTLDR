...
| `SettingsNav` + settings cards | custom (`components/feedtldr/settings/`) | Settings is a page (`/app/settings/*`): flat full-height nav panel (w-64, border, no shadow, 8px inset, sticky) under a "Back to summary" link; group labels are xs semibold muted, items are ink pills. Sections are bordered soft-shadow cards with shape-matched skeletons; the AI-prompt and Accounts cards fill the viewport height on desktop and flex inside. AI prompt is read-only until Edit, with an unsaved-changes guard (native warning on refresh, save/discard/stay dialog on in-app navigation). Mobile: horizontal pill strip |
| `ChatPanel` | custom | Persistent chat on `/app`, layered Dia-style: the chat surface is the flat white base layer (borderless, edge to edge, user-resizable width), and the whole app column (glass bar + summary) lifts in front of it as a rounded card (8px inset, `rounded-card`, `shadow-card`, inner scroll). Kept mounted while closed so the conversation survives reopening; full-screen overlay under `lg` |
