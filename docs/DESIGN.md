...
- Route transitions: /app and /app/settings play a 200ms fade + 8px rise on entry, both directions.
...
| `AppBar` | custom | On `GlassHeader`: avatar (Google `photoURL` or initials, bordered) + settings gear (tooltip, 45deg hover rotation) left; Re-generate primary + AI chat outline right (labels collapse to icons under `sm`) |
...
| `SettingsNav` + settings cards | custom (`components/feedtldr/settings/`) | Settings is a page (`/app/settings/*`): flat full-height nav panel (w-64, border, no shadow, 8px inset, sticky) under a "Back to summary" link; group labels are xs semibold muted, items are ink pills. Sections are bordered soft-shadow cards with shape-matched skeletons; the AI-prompt and Accounts cards fill the viewport height on desktop and flex inside, with `min-h-96` and `min-h-120` floors respectively so short windows scroll instead of collapsing their contents. AI prompt is read-only until Edit, with an unsaved-changes guard (native warning on refresh, save/discard/stay dialog on in-app navigation). Mobile: the nav collapses into a full-width outline button labelled with the current page that opens a near-full-width left Sheet drawer (`w-5/6`, `shadow-overlay`, full height); picking a page navigates and closes the drawer |
| `ChatPanel` | custom | Persistent chat on `/app`, layered Dia-style: the chat surface is the flat white base layer (borderless, edge to edge, user-resizable width), and the whole app column (glass bar + summary) lifts in front of it as a rounded card (8px inset, `rounded-card`, `shadow-card`, inner scroll). Kept mounted while closed so the conversation survives reopening; full-screen overlay under `md` (the app's structural mobile gate — tablets keep the desktop layout) with the summary column made invisible behind it (nothing peeks through around the keyboard), composer padded for the iOS safe area, no autofocus on touch devices |
...
| `PostPreviews` | custom | Wraps `SummaryProse`: clicking a source link (matched by status id from source data) opens a light popup card beside the link — no scrim, 150ms fade/zoom — with the full post verbatim and one primary CTA ("Open the post on X", new tab) instead of navigating; outside click / Escape / scroll dismisses; unmatched links keep default behavior |
...
- **Chat** — persistent right side panel on `/app` (no separate route): a full-height floating white rounded card beside the app column — opening it pushes the app bar and summary content left together on desktop (drag-resizable width); full-screen overlay on mobile that fully replaces the summary; message list + composer with context cards, credits per message noted, clear-chat, closable at any time with history kept while on the page.
