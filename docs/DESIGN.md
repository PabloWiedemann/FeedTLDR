...
| `SettingsNav` + settings cards | custom (`components/feedtldr/settings/`) | Settings is a page (`/app/settings/*`): flat full-height nav panel (w-64, border, no shadow, 8px inset, sticky) under a "Back to summary" link; group labels are xs semibold muted, items are ink pills. Sections are bordered soft-shadow cards with shape-matched skeletons; the AI-prompt and Accounts cards fill the viewport height on desktop and flex inside. AI prompt is read-only until Edit, with an unsaved-changes guard (native warning on refresh, save/discard/stay dialog on in-app navigation). Mobile: the nav collapses into a full-width outline button labelled with the current page that opens a near-full-width left Sheet drawer (`w-5/6`, `shadow-overlay`, full height); picking a page navigates and closes the drawer |
